...
- If Copilot CLI is installed but no GitHub token is found, check for `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `GITHUB_TOKEN` environment variables and guide the user to set one up.
...
- If Copilot is installed but not authenticated, preserve the guidance to configure a GitHub token via `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `GITHUB_TOKEN`.
